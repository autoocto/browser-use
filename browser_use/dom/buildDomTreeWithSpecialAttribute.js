  ...
  function getXPathTree(element, stopAtBoundary = true) {
    ...
    while (currentElement && currentElement.nodeType === Node.ELEMENT_NODE) {
        ...
        for (const attr of locatorAttributes) {
            const attrValue = currentElement.getAttribute(attr);
            if (attrValue) {
                xpath = `//*[@${attr}='${attrValue}']`
                matchingElements = document.evaluate(xpath, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
            ...
            // If multiple elements are found with the current locator, fall back to the next strategy
            if (matchingElements.snapshotLength > 1) {
                continue; // Move to the next locator strategy
            }
        }

        // Priority 2: Check for class names starting with 'trk_'
        const classList = currentElement.classList;
        if (classList && classList.length > 0) {
            for (const className of classList) {
                if (className.startsWith('trk_')) {
                    const tagName = currentElement.nodeName.toLowerCase();
        ...
        // Priority 3: Check for aria-label
        const ariaLabelContent = currentElement.getAttribute("aria-label");
        if (ariaLabelContent) {
            const tagName = currentElement.nodeName.toLowerCase();
            xpath = `//${tagName}[@aria-label='${ariaLabelContent}']`;
            xpathCache.set(element, xpath);
            return xpath;
        }

        // Priority 4: Check for text content
        ...
        // Priority 5: Default to current logic
